...
class DriveClientV1 extends UrlFetchJsonClient {
    ...
    /** Move a file to a different folder (e.g. from My Drive to a Shared Drive folder).
     *
     * @param fileId The ID of the file (may be a shortcut)
     * @param targetFolderId The ID of the target folder
     * @return {Promise<*>} The updated file metadata
     */
    async moveToFolder(fileId, targetFolderId) {
        const resolvedFileId = await this.resolveFileId(fileId);
        const file = await this.getFile(resolvedFileId, 'parents');
        const currentParents = (file.parents || []).join(',');

        const query = DriveClientV1.buildQuery({
        ...
        return await this.patchJson(
            `https://www.googleapis.com/drive/v3/files/${resolvedFileId}${query}`,
            {}
        );
    }
