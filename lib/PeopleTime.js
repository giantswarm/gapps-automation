...
class PeopleTime {
    ...
    /** Returns a new PeopleTime instance shifted by the specified amount of hours (switching days). */
    addHours(hours) {
        const date = new Date(Date.UTC(this.year, this.month, this.day, this.hour, 0, 0, 0)
            + (hours * 60 * 60 * 1000));

        return new PeopleTime(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), date.getUTCHours());
    }
