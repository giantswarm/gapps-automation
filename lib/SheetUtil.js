...
class SheetUtil {
    ...
    static mapRows(rows, columns = undefined) {
        ...
        for (const header of rows[0]) {
            ...
            let id;
            do {
                id = ('' + header || '').trim().toLowerCase().replaceAll(/[^a-z0-9]+/g, '_') + suffix;
                suffix = '' + (+suffix + 1);
            }
            while (fields.includes(id));

            fields.push((!Array.isArray(columns) || !columns.length || columns.includes(id) || columns.includes(header)) ? id : null);
        }
    ...
    static getSheetData(spreadsheet, sheetName, columns = undefined) {
        ...
        return columns != null ? SheetUtil.mapRows(rows, columns) : rows;
    }
}
